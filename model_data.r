...
{##--DO ALL
  ...
  {##--DO ALL
    ...
    path1 <- "D:/1. PALLABI/PHD/ISRO/Program/Using data/sim"
    WD <- "D:/1. PALLABI/PHD/ISRO/Program/Using data/Results"
    ...
    d=load("D:/1. PALLABI/PHD/ISRO/Data/ISRO_Data.RData")
    
    
    #niters <- 30000
    J <- nrow(T.mat) # No. of missions
    K <- ncol(T.mat) # No. of phases
    TC=T.mat
    ...
    ##=========================
    w.array1=aperm(w.array,c(3,1,2))
    dim(w.array1)
    dimnames(w.array1) = list(paste0("Bugs",1:n), paste0("Missions",1:J), paste0("Phases",1:K))
    
    # w.array2 <- array(0, dim= c(N-n,J,K),
    #                 dimnames = list(paste0("Bugs",(n+1):N), paste0("Missions",1:J), paste0("Phases",1:K)))
    # dim(w.array2)
    u0 <- c(rep(0, n))
    # u0 <- c(rep(0, n),rep(1,(N-n)))
    #w.arr0=abind(w.array1,w.array2,along = 1)
    w.arr0=w.array1
    # dim(w.arr0)
    
    data.full0 <- cbind(array(w.arr0,c(n,(J*K))),u0)
    ...
    #S.aug <- c(S0[detected0], S0[!detected0], rep(lam0, M-N))
    ...
    risk <- 1-exp(-(S.aug^1.5)/TCmax) # M x 1
  ...
  sizeBiased <- function(w.arr,y.mat,u,data,TC,
    ...
    for(iter in 1:niters) {
      ...
      #lam0.cand=rgamma(1,shape = 50,rate=0.5)
        ...
        #priorS <- sum(dpois(S, lambda = lam0, log = TRUE))+dgamma(lam0,shape = 50,rate=0.5,log=TRUE)
        ...
        #priorScand <- sum(dpois(S, lambda = lam0.cand, log = TRUE))+dgamma(lam0.cand,shape = 50,rate=0.5,log=TRUE)
      ...
      for(ii in 1:M) {
        ...
        risk.cand.i <- 1-exp(-(Scand[ii]^1.5)/TCmax)
        
        p.cand.i <- c(risk.cand.i * probs / sum(probs), 1 - risk.cand.i)
        llcand.i <- llfn.ind(ww.arr[ii,,],uu[ii], p.cand.i, z[ii])
        
        #priorScand <- dnbinom(Scand[ii], size = (lam0^2) / (varS - lam0), mu = lam0, log = TRUE)
      ...
      # lam0.cand=rgamma(1,shape = 50,rate=0.5)
      # priorLamcand=dgamma(lam0.cand,shape = 50,rate=0.5,log=TRUE)
      # priorLam=dgamma(lam0,shape = 50,rate=0.5,log=TRUE)
      # priorSupdate <- sum(dnbinom(S, size = lam0^2 / (varS - lam0), mu = lam0, log = TRUE))
      # priorScandupdate <- dnbinom(S, size = lam0.cand^2 / (varS - lam0.cand), mu = lam0.cand, log = TRUE)
      # if(runif(1) < exp( priorScand+priorLamcand - priorS+priorLam ) ) {
      #   lam0<-lam0.cand
      #   naccept.lam0 <- naccept.lam0 + 1
      # }
  ...
  {##--DO ALL
    ...
    plot.mcmc <- function(samples1,samples2,samples3,
      ...
      for(this.par in par){
        ...
        # Traceplots
        plot(samples1[1:niters,this.par], type="l", ylab=this.par,main="Traceplot",col="red")
    ...
    #plot.mcmc(samples1=MCMCout1$out,samples2=MCMCout2$out,samples3=MCMCout3$out,par=c('lam0'),niters=niters,burnin=burnin,if.hist=F)
...
save(g1,g2,g3,g1_S,g2_S,g3_S,gel_diag,gel_diag_S,ess,ess_S,mc_se_ch1,mc_se_ch2,mc_se_ch3,
     file = file.path(outfolder, paste0("diag",ts,".RData")))


# print(PAR.ESTIMATES)
#d1=load("D:/1. PALLABI/PHD/ISRO/Program/Using data/Results/Results_Multinomial_Pallabi/nu_1_S_NB/mcmcsamples_231205_145940.RData")
# apply(MCMCout1$out,2,geweke.diag,frac1=0.25,frac2=0.25)
# geweke.plot(as.mcmc.list(MCMCout1$out[,3]))
#dg=load("D:/1. PALLABI/PHD/ISRO/Program/Using data/Results/Results_Multinomial_Pallabi/Results_Multinomial_Snegbinom_SperBug_231104_161855/diag231104_161855.RData")
...
mean_chains=data.frame("psi"=c(mean(MCMCout1$out[25000:niters,2]),
                               mean(MCMCout2$out[25000:niters,2]),mean(MCMCout3$out[25000:niters,2])),"N"=c(mean(MCMCout1$out[25000:niters,3]),
                                                                                                            mean(MCMCout2$out[25000:niters,3]),mean(MCMCout3$out[25000:niters,3])),"lam1"=c(mean(MCMCout1$lam_out[1,25000:niters]),
                                                                                                                                                                                            mean(MCMCout2$lam_out[1,25000:niters]),mean(MCMCout3$lam_out[1,25000:niters])),"lam2"=
                         c(mean(MCMCout1$lam_out[2,25000:niters]),mean(MCMCout2$lam_out[1,25000:niters]),
                           mean(MCMCout3$lam_out[1,25000:niters])),"lam399"=c(mean(MCMCout1$lam_out[399,25000:niters]),
                                                                              mean(MCMCout2$lam_out[399,25000:niters]),mean(MCMCout3$lam_out[399,25000:niters])),
                       "lam400"=c(mean(MCMCout1$lam_out[400,25000:niters]),mean(MCMCout2$lam_out[400,25000:niters]),
                                  mean(MCMCout3$lam_out[400,25000:niters])),"S1"=c(mean(MCMCout1$Sout[1,25000:niters]),
                                                                                   mean(MCMCout2$Sout[1,25000:niters]),mean(MCMCout3$Sout[1,25000:niters])), 
                       "S2"=c(mean(MCMCout1$Sout[2,25000:niters]), mean(MCMCout2$Sout[2,25000:niters]),
                              mean(MCMCout3$Sout[2,25000:niters])),"S399"=c(mean(MCMCout1$Sout[399,25000:niters]),
                                                                            mean(MCMCout2$Sout[399,25000:niters]),mean(MCMCout3$Sout[399,25000:niters])), "S400"=
                         c(mean(MCMCout1$Sout[400,25000:niters]), mean(MCMCout2$Sout[400,25000:niters]),mean(MCMCout3$Sout[400,25000:niters])))

sd_chains=data.frame("psi"=c(sd(MCMCout1$out[25000:niters,2]),
                             sd(MCMCout2$out[25000:niters,2]),sd(MCMCout3$out[25000:niters,2])),
                     "N"=c(sd(MCMCout1$out[25000:niters,3]), sd(MCMCout2$out[25000:niters,3]),
                           sd(MCMCout3$out[25000:niters,3])),"lam1"=c(sd(MCMCout1$lam_out[1,25000:niters]),
                                                                      sd(MCMCout2$lam_out[1,25000:niters]),sd(MCMCout3$lam_out[1,25000:niters])),"lam2"=
                       c(sd(MCMCout1$lam_out[2,25000:niters]),sd(MCMCout2$lam_out[1,25000:niters]),
                         sd(MCMCout3$lam_out[1,25000:niters])),"lam399"=c(sd(MCMCout1$lam_out[399,25000:niters]),
                                                                          sd(MCMCout2$lam_out[399,25000:niters]),sd(MCMCout3$lam_out[399,25000:niters])),
                     "lam400"=c(sd(MCMCout1$lam_out[400,25000:niters]),sd(MCMCout2$lam_out[400,25000:niters]),
                                sd(MCMCout3$lam_out[400,25000:niters])),"S1"=c(sd(MCMCout1$Sout[1,25000:niters]),
                                                                               sd(MCMCout2$Sout[1,25000:niters]),sd(MCMCout3$Sout[1,25000:niters])), 
                     "S2"=c(sd(MCMCout1$Sout[2,25000:niters]), sd(MCMCout2$Sout[2,25000:niters]),
                            sd(MCMCout3$Sout[2,25000:niters])),"S399"=c(sd(MCMCout1$Sout[399,25000:niters]),
                                                                        sd(MCMCout2$Sout[399,25000:niters]),sd(MCMCout3$Sout[399,25000:niters])), "S400"=
                       c(sd(MCMCout1$Sout[400,25000:niters]), sd(MCMCout2$Sout[400,25000:niters]),sd(MCMCout3$Sout[400,25000:niters])))

cv_chains=data.frame("psi"=c(cv(MCMCout1$out[25000:niters,2]),
                             cv(MCMCout2$out[25000:niters,2]),cv(MCMCout3$out[25000:niters,2])),
                     "N"=c(cv(MCMCout1$out[25000:niters,3]), cv(MCMCout2$out[25000:niters,3]),
                           cv(MCMCout3$out[25000:niters,3])),"lam1"=c(cv(MCMCout1$lam_out[1,25000:niters]),
                                                                      cv(MCMCout2$lam_out[1,25000:niters]),cv(MCMCout3$lam_out[1,25000:niters])),"lam2"=
                       c(cv(MCMCout1$lam_out[2,25000:niters]),cv(MCMCout2$lam_out[1,25000:niters]),
                         cv(MCMCout3$lam_out[1,25000:niters])),"lam399"=c(cv(MCMCout1$lam_out[399,25000:niters]),
                                                                          cv(MCMCout2$lam_out[399,25000:niters]),cv(MCMCout3$lam_out[399,25000:niters])),
                     "lam400"=c(cv(MCMCout1$lam_out[400,25000:niters]),cv(MCMCout2$lam_out[400,25000:niters]),
                                cv(MCMCout3$lam_out[400,25000:niters])),"S1"=c(cv(MCMCout1$Sout[1,25000:niters]),
                                                                               cv(MCMCout2$Sout[1,25000:niters]),cv(MCMCout3$Sout[1,25000:niters])), 
                     "S2"=c(cv(MCMCout1$Sout[2,25000:niters]), cv(MCMCout2$Sout[2,25000:niters]),
                            cv(MCMCout3$Sout[2,25000:niters])),"S399"=c(cv(MCMCout1$Sout[399,25000:niters]),
                                                                        cv(MCMCout2$Sout[399,25000:niters]),cv(MCMCout3$Sout[399,25000:niters])), "S400"=
                       c(cv(MCMCout1$Sout[400,25000:niters]), cv(MCMCout2$Sout[400,25000:niters]),cv(MCMCout3$Sout[400,25000:niters])))

t(round(mean_chains,4))
t(round(sd_chains,4))
100*t(round(cv_chains,4)) 

mean(c(MCMCout1$remaining_size[burnin:niters],MCMCout2$remaining_size[burnin:niters],MCMCout3$remaining_size[burnin:niters])<25)